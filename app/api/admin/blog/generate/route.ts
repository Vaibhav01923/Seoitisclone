...
MEASUREMENT: every tracked prompt is sent to 5 AI answer surfaces — ChatGPT, Claude, Gemini, and Perplexity via their own APIs, plus Google's AI Overview via live SERP scraping (not all queries trigger an AI Overview — no-show is a real, recorded outcome, not a failure). For each response, RankOnGeo checks whether the brand is mentioned and, if the answer is a ranked/numbered list, at what position. Per-engine visibility score = % of tracked prompts where the brand was mentioned by that engine; the overall score is the average across engines. Competitor mentions are extracted the same way, so "who's beating you and where" is a byproduct of the same scan, not a separate lookup.

CADENCE: a full scan across all engines runs every 3 days. Each tracked prompt adapts its own schedule — after 7 consecutive scans where every engine that answered mentioned the brand, that prompt drops to roughly weekly checks (saving scan volume on queries that have stabilized); a single miss puts it straight back on the every-scan list. This is automatic and not visible as a setting the user tunes.
